...
export default function CreateBooking() {
  ...
  // The slot passed from Tasks page (if any).
  const preSelectedSlot = location.state?.selectedSlot || null;
    ...
    const fetchSlots = async () => {
      try {
        ...
        const config = user?.token
          ...
          : {};
        const response = await axiosInstance.get('/api/slots', config);
        const allSlots = response.data || [];
        setSlots(allSlots.filter((s) => s.status === 'available'));
      } catch {
        setErrorMessage('Failed to load slots. Please go back and try again.');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  // Create a booking in MongoDB for the selected slot.
  ...
  const getRemainingText = (slot) => {
    const cap = Number(slot.capacity);
    if (cap === 0) return 'Full';
    if (cap === 1) return '1 spot left';
    return `${cap} spots left`;
  };
      ...
      <div className="relative w-[390px] min-h-[844px] bg-white px-[14px] pt-[10px] pb-[110px] flex flex-col">
        ...
        <div className="mt-[16px] flex justify-center">
          <div className="rounded-[8px] bg-[#5c9df5] px-4 py-2">
            <h1 className="text-[18px] font-semibold text-white">Create New Booking</h1>
          </div>
        </div>
        ...
        <div className="mt-[18px] space-y-[18px]">
          {isLoading ? (
            ...
            slots.map((slot) => {
              const isFull = Number(slot.capacity) === 0;
              const isSubmitting = submittingId === slot._id;
              return (
                <div key={slot._id}>
                  <div className="rounded-[8px] border border-[#d3d3d3] bg-[#f7f7f7] px-[16px] py-[14px]">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <p className="text-[18px] font-bold leading-none text-[#303030]">{slot.dateLabel}</p>
                        <p className="mt-[8px] text-[18px] font-bold leading-none text-[#4b4b4b]">{slot.timeLabel}</p>
                      </div>
                      <button
                        type="button"
                        disabled={isFull || isSubmitting || submittingId !== ''}
                        onClick={() => !isFull && handleBookNow(slot)}
                        className={`h-[40px] min-w-[100px] rounded-[8px] px-3 text-[18px] font-semibold text-white ${
                          isFull ? 'bg-[#cfcfcf]' : 'bg-[#45d463]'
                        } ${submittingId !== '' ? 'opacity-70' : ''}`}
                      >
                        {isSubmitting ? 'Saving...' : isFull ? 'Unavailable' : 'Book Now'}
                      </button>
                    </div>
                  </div>
                  <p className={`mt-[6px] pl-[10px] text-[16px] ${isFull ? 'text-[#ff4747]' : 'text-[#6acb5f]'}`}>
                    {getRemainingText(slot)}
                  </p>
                </div>
              );
            })
